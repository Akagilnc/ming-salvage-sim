...
export async function runEpicLayeredPipeline({ args, Bash, agent: agentRunner, log }) {
  ...
  for (const [layerIndex, layerPlan] of discovery.orderedPlan.entries()) {
    ...
    const sliceBaseContext = buildSliceBaseContext(normalizedArgs, mergeQueue, startupTargetHead, existingFamily?.familyHead);
  ...
  const handoffBase = {
    ...
    merged: mergedSlices,
    dirty: [],
    // Carry the prior user rulings forward so the WHOLE handoff round-trips as relaunch args: a
    // needs-user/unconverged relaunch must not drop the already-dismissed decision findings (else the
    // same finding re-HALTs) or the cross-segment decision text. The main session adds any new rulings.
    dismissals: normalizedArgs.dismissals,
    decisions: normalizedArgs.decisions,
    flags: shipFlags
  };
...
function buildSliceBaseContext(normalizedArgs, mergeQueue, startupTargetHead, continuationFamilyHead) {
  const lastMerged = mergeQueue.at(-1);
  // Case 1: slices already merged this segment -> base on the family branch HEAD (the last merge).
  ...
  // Case 2: continuation relaunch, nothing merged yet THIS segment but prior segments did -> base the
  // first todo slice on the EXISTING family HEAD via the family-branch path (so it sits on top of the
  // already-merged blockers). baseAtStart/diffBase keep the real startup target head.
  if (continuationFamilyHead) {
    return {
      familyBranch: normalizedArgs.familyBranch,
      baseBranch: normalizedArgs.familyBranch,
      baseRef: continuationFamilyHead,
      startupTargetHead,
      targetBranch: normalizedArgs.targetBranch,
      mergeQueue: []
    };
  }
  // Case 3: first segment, nothing merged anywhere yet -> base on the startup target branch.
...
function normalizePipelineArgs(rawArgs, epicIssueNumber) {
  ...
  // accept the prior handoff's `merged` field as the continuation alias for mergedNumbers, so the
  // whole handoff object round-trips as relaunch args (alongside baseAtStart -> startupTargetHead).
  const mergedEntries = normalizeMergedEntries(objectArgs.mergedNumbers ?? objectArgs.merged);
  return {
...
function assertShellSafeRef(value, name) {
  if (value === '' || value.trim() === '') {
    throw new Error(`epic-orchestrator: ${name} must not be empty or whitespace-only (an empty ref would reach git/gstack-ship and fail opaquely).`);
  }
...
async function runFamilyReview({ Bash, runner, familyWorktree, familyBranch, epicIssueNumber, maxReviewRounds, verifyCommands, diffBase, dismissed = [] }) {
  ...
  for (let round = 1; round <= maxReviewRounds; round += 1) {
    ...
    const preFixHead = (bashText(await Bash(`set -euo pipefail\n# family-review pre-fix HEAD\ngit -C ${shellQuote(familyWorktree)} rev-parse HEAD`)) || '').trim();
    await runner({
...
function familyReviewFindings(report, reviewerName, statusRequired) {
  ...
  // Sanitize BEFORE the length check: drop falsy / non-object garbage so a {status:"failed",findings:[null]}
  // (or [garbage]) does NOT short-circuit as "has findings" and then silently vanish downstream (routeFindings
  // drops the garbage) — it must fall through to the non-pass-status contradiction check below.
...
function buildFamilyReviewPrompt(epicIssueNumber, familyBranch, round, diffBase) {
  return [
    ...
    'REVIEW ONLY — HARD CONSTRAINT. Do NOT modify, create, rename, or delete any file in the family worktree; do NOT commit, push, or run tests / builds / any command that writes to the worktree (they would dirty it before Ship). Read-only inspection (git log/diff, grep, reading files) is fine. Return findings only.',
...
async function runPerSliceReview({ Bash, worktreePath, commit, plannedSlice }) {
  ...
  const agy = parseReviewerJson(
    await Bash(`set -euo pipefail\ncd ${shellQuote(worktreePath)}\n# reviewer=agy; canonical invocation — '-p --sandbox' swallows --sandbox as the print value and the old diff-only flag is not a real agy option; both would break the leg. agy auto-degrades to diff-based review on the hidden .epic-orchestrator worktree. Review instructions + JSON schema ride stdin before the diff (agy is agentic -> hard read-only constraint) so it returns parseable reviewer JSON, mirroring the family agy leg.\n{\n  printf '%s\\n' 'REVIEW ONLY — HARD CONSTRAINT. Do NOT modify, create, rename, or delete any file; do NOT run commands. Output only the review JSON.'\n  printf '%s\\n' 'Review the following git diff for correctness regressions.'\n  printf '%s\\n' 'Return only one JSON object on stdout with shape {"status":"passed"|"failed","findings":[{"id":...,"classification":"mechanical_bug"|"choice","claim_quote":...,"location":...}]}. Do not include markdown or prose outside the JSON object.'\n  printf '%s\\n' 'Diff stat for context:'\n  git diff --stat ${shellQuote(`${commit}^`)} ${shellQuote(commit)}\n  printf '%s\\n' 'Full diff:'\n  git diff ${shellQuote(`${commit}^`)} ${shellQuote(commit)}\n} | agy --sandbox --print ''`),
    'agy'
  );
  const reviewers = [
