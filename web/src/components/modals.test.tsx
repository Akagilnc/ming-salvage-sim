...
import { AudienceArchiveModal, ChatModal, EdictModal, HistoryModal, ReportModal, parseLeadingStageDirection } from "./modals";
...
describe("AudienceArchiveModal — read-only scene archive", () => {
  ...
  it("归档场卷只按大臣持久清单高亮并复用 organic 归一化", async () => {
    ...
    vi.stubGlobal("fetch", vi.fn().mockImplementation((url: string) => Promise.resolve({ ok: true, json: async () =>
      url === "/api/history/turns" ? { turns: [{ kind: "night", night_id: 31, title: "旧场卷" }] } : { messages: [
