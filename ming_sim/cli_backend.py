    ...
    if compact:
        ...
        negated_approval_hit = any(
            ...
            for token in (
                ...
                "不必准奏", "不必照准", "不必照办", "不必依卿", "不必如此",
                "不用准奏", "不用照准", "不用照办", "不用依卿", "不用如此",
                "无须准奏", "无须照准", "无须照办", "无须依卿", "无须如此",
            )
        )
        approve_hit = (
...
def enrich_initiative_effects(title: str, stage: str = "", llm_config: Any = None) -> Dict[str, Any]:
    ...
    prompt = (
        ...
        '    "new_armies": [{"id":"英文小写id","name":"军名","owner_power":"ming","manpower":兵额(整数,如18000),"commander":"主将姓名或空","station":"驻地","troop_type":"步/骑/水/车营","火器":0到100整数(火器局/神机营/火器新军给高),"随军大炮":0到12整数门数(炮营/红夷炮新军给几门)}],   // 月饷由引擎按 manpower 自动核算，勿列饷额\n'
        ...
        "- 练兵/募营/建新军 → new_armies（给合理兵额/主将/驻地；月饷由引擎按 manpower 自动核算）\n"
