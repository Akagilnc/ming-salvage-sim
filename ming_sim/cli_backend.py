...
def classify_cli_action_intent(
    ...
    prompt = (
        ...
        "（新建不受此限）。非妃嫔不要硬判调教。\n"
        "交办·责成 vs 当场问对 边界（ADR 0042，#1565）：\n"
        "- 交办·责成 = 需在本次召对之后仍要继续执行的政务工作（回合结束后仍在进行），"
        "这是真正的交办/责成，务必产出后续回合的 ongoing 工作。\n"
        "- 当场问对 = 大臣当场被点名解释/比较/列数，或据实回奏/分策回奏/开列回奏的，"
        "属普通对话 → 动作类型=无（kind none），不是交办，零 staging。典型问对举例：\n"
        "  · 杨卿，太仓实存与关宁、陕西边饷核到哪一步？盐课积引与畿辅清丈何者可先试？分策回奏\n"
        "  · 郭卿，改元之后钱粮如何撑过春月？关宁与陕西何者当先？太仓实存与刚性支出开列回奏。\n"
        "  · 黄阁老，入春边事与朝局何者最急？关宁军心、陕西赈抚、厂卫旧线，内阁据实回奏。\n"
        "  这类「在座即问、当场要数/比较/回奏」的提问，统一判为无，坚决不得上升为交办。\n"
        "交办·责成承诺契约：\n"
