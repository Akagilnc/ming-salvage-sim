...
def classify_cli_action_intent(
    ...
    prompt = (
        ...
        "同一事项同一目标的重复表示不得重复候选。"
        "拟旨与其任免/拨帑等机械载荷候选可按既有契约并存，以同时保留旨稿和实际动作；"
        "拟旨本身已覆盖该道旨的颁布——不得再为同一拟旨事项并出交办·责成；"
        "交办·责成只用于与拟旨彼此独立、须离殿后另行履行的事项。"
        ...
        "- 离殿后继续履约的整体语义优先于回奏形式：凡要求召对结束后继续办理，动作类型"
        "必须填交办·责成，不能填无；即使同时要求据实、分策或开列回奏也一样。例如"
        "退朝后核查各省库、三日内逐项开列回奏，是会后交办，动作类型必须填交办·责成；"
        "日级期限无法换算为月数或回合，不填写期限月数或截止回合。\n"
        "交办·责成承诺契约：\n"
