...
def build_minister_tools(character: Character, context: CourtContext,
    ...
    def secret_order(
        ...
        if act == "issue":
            return _secret_order_issue(title, content, tags_json, assignee, deadline_months, excluded_names_json)
        if act == "progress":
    ...
    def _secret_order_issue(title: str, content: str, tags_json: str = "[]", assignee: str = "", deadline_months: int = 0, excluded_names_json: str = "[]") -> str:
        ...
        try:
            ...
            deadline = 0
        return f"__secret_order__{json.dumps({'title': t, 'content': c, 'tags': tags_clean, 'assignee': real_assignee, 'deadline_months': deadline, 'excluded_names': excluded}, ensure_ascii=False)}"
