...
def invoke_highlight_judge(reply: str, llm_config: Any) -> str:
...
def judge_highlights(
    ...
    invoke: Callable[[str, Any], Any] = invoke_highlight_judge,
) -> List[str]:
    """Return silently and on time for exceptions, malformed output, and timeout."""
    results: "queue.Queue[Any]" = queue.Queue(maxsize=1)

    def work() -> None:
        try:
            results.put(invoke(reply, llm_config), block=False)
        except Exception as error:  # best-effort decoration must never break chat
            try:
                results.put(error, block=False)
            except queue.Full:
                pass

    threading.Thread(target=work, daemon=True, name="audience-highlight-judge").start()
    try:
        result = results.get(timeout=max(0.0, float(timeout)))
    except queue.Empty:
        return []
