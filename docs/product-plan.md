...
- 军队字段：supply / morale / training / equipment / arrears / mobility / loyalty / manpower / salary_rate（月饷由 army_needed=ceil(manpower×salary_rate/10000) 派生；#173 维护费列已删）。

### 简化

- **数值少而强**：全局面板只显示钱粮 / 民心 / 皇威；动乱、士绅阻力、欠饷等落在各省 / 各军队字段，只在事件文本、回奏与详情里露出。减少面板数字密度，强化每个数字的解释力。

### 深化

- **联动公式**：当前 `apply_execution_eval` 是 LLM 直接给 delta。深化方向是建立"数值之间的因果链"：
  - 欠饷高 → 士气↓ → 忠诚↓ → 哗变事件触发。
  - 民心低 + 某省动乱（unrest）高 → 地区税收基数↓。
  - 派系 leverage 失衡 → 旨意结果与原意偏差↑。
- **阈值事件**：数值跨阈值自动触发事件（民心 < 30 → "京师粮价上涨"；北直隶 military_pressure > 70 → "后金绕袭传闻"）。挂到 #3 历史事件预置之上，已由 events.json 的 `trigger_gate` 支持地区/军队字段阈值。
