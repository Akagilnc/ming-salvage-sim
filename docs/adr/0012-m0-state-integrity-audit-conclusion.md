...
4. **✅ Q3 契约已冻结（用户 2026-06-15 拍 = fail-loud）→ 转编码（#12，OPEN）**，含两块：
   ...
   - **(b) 缺字段 fail-loud** — 坏 gate 现静默当「条件不满足」：`issues.py:194`（未知 metric_key→`None`）→`:302`（`return False`）。load 期**已有部分校验**（`content.py:106` 验比较式语法 `^(>=|<=|>|<|==)\s*-?\d+$`、坏语法 `raise SystemExit`，故「坏比较语法」这类**已 fail-loud**——修正初稿「零 schema 校验」之误）；剩缺口 = ① 未知/拼错 metric_key 不校验、② 文本相等 gate load/runtime 不一致（runtime `_gate_passed` 支持 `==文本`，load 只放数值、文本会被拒）。**裁断（已拍）：畸形 gate = fail-loud**（未知 key / 非法表引用 / 坏语法均报错或拒收，符 ADR 0005）；**保留区分**：合法但未满足仍 `return False`、只对畸形响亮。**trigger_gate 错属静态 content schema 错**（非 ADR 0008 LLM/代码二分）→ 走 **load-time 校验**（content.py，已部分）、**不走 `RejectionCollector`**（LLM-delta 专用）。
