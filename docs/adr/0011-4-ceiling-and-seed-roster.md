...
| 既得利益（夺产 / 夺爵 / 逼捐 / 清丈抗免田 / 裁宗禄） | **seg(leverage)** 50–85 | T2（base 最低；唯一受 leverage seg 段调制，见 D4-2） | 李国瑞逼捐忧惧死 → 退银复爵、薛国观助饷代理人赐死 |
...
**互斥收口（原 ⚠️ CMR，P3-7）**：祖制 tag 用「换行路由」（改查哪行）、宗室 / 勋戚用「加数」、leverage 用「`seg` 段函数」（均改值）；两套**不冲突**——祖制换行（`reason_code ∉ 依律`）与 D4-4 翻轴换行（`reason_code ∈ 依律`）在 `reason_code` 机读字段上**严格互斥**，任一动作至多命中一条换行、无优先级问题；坐实即整条命门 tag 不命中、加数 modifier 一并归零（见 D4-4 a）。
...
**与血债的关系（钉清，对齐 0011-2 D2-7）**：「出路恒可达」≠ resistance → 0。翻轴塌的是**该具体命门动作**的 ceiling / 命门 floor；外压杠杆降的是**华夷命门 floor 本身**（D4-3 决定性；务实派 dynamic 软化为次要项、单独不足）；**血债 floor 是对该派的持久代价**（冻土，两出路都不清）。0011-2 D2-7 已钉：血债 floor 臂设 `cap < 依律出路阈(~35)`，令 unbounded 血债能挡私意高墙、但挡不死出路动作 → 出路恒可达对 unbounded 血债仍成立。**议和侧**：议和累 blood_debt（冻土）、不触底任何派 sat/lev，故净负窄路不变式不被破（0011-3 D3-4）。
...
**决定**：base ceiling（95/85/82/72/40/35/30）+ modifier 量（祖制 +5 / 勋戚 +8 / leverage→`seg(leverage)` 段函数 D4-2）= **首版**，随矩阵 α/β playtest 调参（镜像 spike G1-G22 方法学：独立 oracle + 末态硬期望 + mutation 自验）。
...
**单一真源已闭合（原 ⚠️ CMR，P2-1）**：0011-2 D2-7 公式行 + D2-7 floor 注 + 依赖 substrate 节的 `命门合法性floor` / `ceiling` 引用源**已从 dig-9 升级到本 ADR D4-3**（同步完成），语义一致（dynamic 低时 floor 托到 ceiling、国本之争原型）。dig-9 保留为底稿出处。
