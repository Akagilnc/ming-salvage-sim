...
Status: Accepted（2026-06-15；**务实版**——用户拍板：复用 issue 不另造、补饷喂现有 arrears 还款池且含损耗、未来复试最小化。地基机制经实证调查（file:line）锚定；设计评审已收敛并进入实现期。file:line 为指示性、以函数名为准。）
...
**承诺涵盖面（2026-06-17 grill #187 厘清）**：承诺载体不止「圣旨拨款」类，也涵盖**持续性规避/治理动作**——如「安抚毛文龙」（命臣安抚、效果软判抬其 `loyalty`，直到 `loyalty >= 阈值` 才算安抚成）。**判据 = 持续努力 → commitment issue；一锤子（赏赐 / 一笔抚恤）→ 一次性 delta（`economy_move` 等）、不建 issue**。规避动作走承诺载体后，其「完成条件」即一条 `stop_condition`（如 `character.毛文龙.loyalty >= 65`），与对应历史事件的 `trigger_gate` 同读一个字段（一个达标即收承诺、一个未达即触发事件；详见 #174 / #187）。
