...
## ✅ 用户拍板（2026-06-14，全认 Claude lean）
1. **ceiling表 ✅ 跟矩阵42格一起拍**(co-依赖)；"出路杠杆恒可达"硬不变式先锁。【唯一真缺的设计块=随矩阵值 sub-spec 一起填】
2. **第一刀 ✅ 严格只颁布关 顺颁/打回**，执行层四态 defer 第二刀。
3. **中旨"一意孤行"第一刀 ✅ 给但映射"硬推=必碰壁打回"**(埋伏笔，第二刀接真中旨闸)。
4. **合成口径 ✅ 逐层短路出 blocked_layer**(实现取定，邸报/破局都要"卡哪层")。
5. **kinship 改动 ✅ 血债 sub-ADR 带它、四层票拟另起 sub-ADR**(评审批次划分)。

**四层票拟定稿。actor 博弈设计阶段(决定1-9)全收口。**
