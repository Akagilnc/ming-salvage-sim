...
3. **重跑契约(三件缺一不可,r1/r2 评审并发指出,全部列入 PR1 验收)**:
   ...
   〔2026-09-09 后出注记：[#1822](https://github.com/Akagilnc/ming-salvage-sim/issues/1822)——分段后不另存转译产出为重试真源，恢复只看最后一段落没落账（落了起下一段、有文字没落账重跑该段转译、丢一半从上一段续演）；resolve_context ready=1 重放机械在该形状下退役，pre_settle 自事务 + settling 相位 + 内存态重载不变。〕
