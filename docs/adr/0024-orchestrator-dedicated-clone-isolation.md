...
2. **把 *prune* 职责交回 Sandcastle，但常驻 worktree 的存废不交给 `.close()`**：① 删掉 `cleanResidueAt` 的 repo 级 `git worktree prune`（既重复 Sandcastle 自身、又危险）；旧“保留 `reset --hard HEAD` + `clean -fd` 片内清场”已被 ADR 0127 废止，非授权回收路径不清场；② **常驻 slice / family worktree 不得用 `await using` / `.close()` 做常规清理**——Sandcastle 的 `Worktree.close()` 在 worktree 干净时 `remove()` 它（`dist/index.js` close 实现），而常驻 worktree 是 commit 真源 + 崩溃复用真源，删了就破续跑。删除只有 #868/#869 定义的两个显式 Closure / Reclamation 授权入口；准确条件只读其当前真源与 ADR 0127。「回归原生」仅指把 prune 交回库，**不**指把常驻 worktree 的存废交给库。
