...
1. **只调度现成子片。** Issue Admission Action 的 family 分支在首次启动与每次 re-entry 读取 live native membership、状态、标签和依赖，完成过滤与 cycle 处置并产出 GitHub 候选集和依赖交通事实；Canonical Delivery Flow 随后调用 Scene Provisioning / Recovery，恢复当前 flow 已有的 scene inventory 并检查新候选。只有 Scene Provisioning / Recovery 查询 locator，并分别报告“无既有 scene”“scene 属于当前 flow”“scene 属于其他 flow”三种交通事实；Canonical Delivery Flow 消费这些事实并归结 create、recover 或 local-park fixed position，Generic Runner 只执行该位置，不接收事实、不重复抓取、分类、查询 locator 或亲自建立现场。当前只支持一层 family；仍有 sub-issues 的 child 不作为可运行叶子，递归 family 留待后续设计。
2. **一个父现场、每子片一个独立现场。** Parent base 是唯一家族整合面；每个可运行 child 从当时的 parent base 切出 distinct branch / worktree。直接输入一个无既有 scene、且经 Issue Admission 确认为 open + ready 的叶子 issue 时才走 single；closed/open-unready 无 scene 叶子 quiet skip；已有 scene（含 terminal failure 后保留者）一律重入 owning flow，不虚构 parent base 或建立第二现场。
...
6. **membership removal 是窄取消。** 未合 child 从 native sub-issues 移除后停止未来调度；若其 agent invocation 仍在运行，不杀进程、不删 worktree。实例不再运行后，由 Closure / Reclamation 只删除目标 worktree，保留 branch、Lineage / ledger、日志与 telemetry / 统计，不要求 success 或 normal exit，也不撤销已合代码。
