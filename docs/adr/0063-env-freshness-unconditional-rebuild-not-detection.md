...
使用者只面对一个公开开工入口并只提供 issue 号；该入口在派活前完成上述全部准备，再定位父/子 issue 的唯一工作现场并调用内部流程。`runFamilyDriver` 等组装函数是内部能力，不是第二个公开入口；README 不再要求使用者手工执行 freshness ritual、创建 per-issue driver 或拼装内部参数。
