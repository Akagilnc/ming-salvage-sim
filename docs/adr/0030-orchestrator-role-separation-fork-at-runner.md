...
## 传导范围（pre-0030 标记，实现时同步）

本反转推翻的旧设计仍写在几处 **agent / 人会读的活文件**里，已就地标「pre-0030，见 ADR 0030」+ 指针，实现（#369/#419）落地时一并改，免得实现 agent 照旧设计做：`CLAUDE.md` 的 `## Skill routing`（说 per-slice 评审是 coder 自己的事、无独立 reviewer worker）、`orchestrator/image/souls/reviewer.md`、`orchestrator/src/route.ts` 与 `orchestrator/src/runner.ts` 的 ADR 0026 注释（S2-only 塌缩、S3/S5/S6 deleted）。**0030 实现落地前，这些仍是当前（0026）有效设计**——标记只提示「即将反转 + 指针」，不是说现在就错了。
