...
- 分立判据=**级联不同**：任命（绑名分+顶替派生+政治标记派生）/ 罢黜（清名分+status→dismissed+派系连动；贬义归派系层）/ 调任（旧职解绑+新职绑定，**含降职**——左迁贬谪级联同平调，贬义归派系层）/ 处置（status 迁移族：下狱/流放/致仕/放归/赐死/卒/起复/昭雪/夺情，原「存殁」更名——该词装不下下狱与致仕）/ 易主（翻 power_id+换名衔+内嵌反噬）/ 册封（后宫专线，前置=candidate）/ 行止（仅去向，见决定 5）。
...
**处置 payload 形态**：目标 `status` 必填 + 具名子动作可选 + `reason_code` 可选。族列举（下狱/流放/致仕/放归/赐死/起复/昭雪/夺情/卒）是常用具名示例，无名迁移以目标 status 为准（S9 出宫、S14 落选即此形）；`invalid_transition` 判据对目标 status 生效。
