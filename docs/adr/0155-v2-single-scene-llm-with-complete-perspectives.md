...
首期由一个 LLM 演整场召对，按人物组织材料后一次给全本次职责可见内容、让它自己阅读取舍，优先还原北极星的连贯演出并减少等待，不采用每人一个模型、导演调用或按需检索往返，也不预建长期记忆整理或摘要机制，待实际出现上下文影响后再讨论。
...
本决定不把世界推演和所有辅助处理合成一次调用，原话及被替代的前案见 [召对取舍依据](../design/game-v2-architecture.md#召对取舍依据)，不做记忆整理的后续裁定见 [记忆供料保持简单](../design/game-v2-architecture.md#记忆供料保持简单)。
