...
- **皇帝下一句明确应允** → 当场 commit 该召对大臣的暂存（不等颁诏）〔召对夜容器下此句被 ADR 0038 取代：应允=夜域「已应允」态、收夜提交，见下「提交时机」取代注〕；
...
- **闸门覆盖聊天写动作**：召对里皇帝没点按钮、只是说话，由 LLM 判出的密令更新/催办/提交核议/记进展、**任免（任命/罢免）**、后宫调教 —— 这类**推断**动作进暂存、过确认才落库。显式「拟旨如下：」按钮/前缀同样先进 `pending_actions(kind=directive)`：大臣以 in-character 方式润色草案并问准否；皇帝拒绝→删暂存，皇帝不回→颁诏 checkpoint 默认同意为 `draft`，皇帝在召对里应允→转成 `turn_directives.status='pending'`，仍走既有准/驳界面〔0049 后出为准：收夜提交即准旨，月末无颁诏复审步〕。显式「密令如下：」仍是权威分类，但密令字段提取保留独立路径。
