...
- **ceiling 敏感度天花板**（真源 = `0011-4` A 部分；dig-9 底稿）：供 D2-7 的 `命门合法性floor` 与 `min(ceiling, dynamic)` 臂；命门度挂 axis-tag（私意 vs 坐实）、不挂目标身份 → **出路恒可达**（三机制，见 0011-4 D4-4：① 有被告命门走翻轴 reason_code=依律 → 轴翻转 → ceiling 塌；② 华夷议和走外压 dynamic 杠杆；③ 纯制度祖制 / 无被告清议走母 ADR 决定6 盘面级净负窄路兜底）已证明。
