...
**信封宪法（收口，2026-07-06）**：runner 只读控制信封——exit code / `findings.length` / 决策门信号位——从不读信封里的字；finding 富内容（severity/位置/修法）经 landing file 在 worker 间直达 coder-fix，不经 runner 判断面；决策 payload 与人的答案对 runner 不透明，runner 纯搬运。任何「读 finding/decision 内容再分叉」的代码都是回归，删。
