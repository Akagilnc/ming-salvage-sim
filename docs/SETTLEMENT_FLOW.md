...
advance_without_edict(state, db, ...)  # prep-only，恒 return False
  前半段已完成（turn_phase ∈ FRONT_HALF_DONE_PHASES）→ 直接 raise：
    结算欠账不可退朝跳过，只能续跑 / 重新推演（awaiting 态则先亲裁）——ADR 0008 决定 6
  capture_month_open_snapshot + auto_close_open_night(...)
    # #498 退朝遇开夜顺势自动收夜；#542 scene_registry 调用方所有
  不再推进回合 / 不写月档 / 不跑财政——结算一律归 resolve 全链（#1416 快路月档随分支消亡）
```
...
- **推进尾同款**（settle_with_delta / simulator 失败 fallback 仍归 settle 核）：atomic 内同笔做完「清 resolve_context → next_period → 相位复位 summoning → save_state」。#1274：advance_without_edict 不再是推进尾（prep-only），无旨月推进只经 settle_with_delta。
