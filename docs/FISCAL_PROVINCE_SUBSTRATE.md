...
>
> **效果 = 读事件结局算增量**:读事件账(哪些加饷结局==已准且未被议停)→ 定在征饷集 {辽/剿/练} → 算各省 settle.p(三饷应征=Σ在征分量、起运联动、火耗派生);**幂等 set-to-target**(每 tick 重算到在征集目标、不逐月叠加)。#259 定义事件+结局集+读结局效果,不实现圣旨路由(=#150/#260)。
>
