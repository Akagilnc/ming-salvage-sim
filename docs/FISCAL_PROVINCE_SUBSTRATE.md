...
下表 = **陕西 #579 一手核后模板/锚**。#585 收尾时全 15 布政司已按同法完成首轮一手核/重标落库；辽东按专属军饷源，皮岛/东江无《万历会计录》对应源、维持军饷漏斗口径。详细原文转录、折算式与扫描核验记录见各省 `content/regions.json` 的 `settle._meta`。

| 字段 | 旧占位 | 真实锚(源) | 重标值 |
|---|---|---|---|
| `正赋应征` | 60/月(720万/年) | 《万历会计录》卷九陕西布政司田赋:本色实征麦688,647.2416石、米1,044,943.1241石;同卷近例税粮每石折银三钱五分 | **5.0563/月**=((麦+米)×0.35)/12/10000 |
| `三饷应征`(辽饷) | 10/月 | 陕西一手官民田2,929.20151万亩 × 九厘0.009(开局)→崇祯四年1631 一分二厘0.012,Src[3] | **2.1969011325/月**=开局1627.10 九厘值(1631 升后由事件抬到 4/3 倍,见下注) |
| `起运定额`(=送京/解边起运上限,**封顶正赋+三饷的合并起运额**;封顶值是本定额、非税基,见 `起运池=min(实征,定额)`) | 40/月 | 《万历会计录》卷九按语「陜西歲額，悉留本省，爲四藩五鎮之費」→正赋起运基线=0;**三饷须全额解京/边**→总 cap 覆盖在征三饷 | **2.1969011325/月**=正赋起运基线0 + 开局辽饷;**须随三饷事件同步上调**(1631→辽饷4/3倍、剿/练再加),否则超额三饷被困省内、违「三饷全解」史实(gemini PR#113) |
...
| `Due.宗禄`(秦藩) | 4/月 | 《万历会计录》卷三十二宗藩禄粮:秦藩、西宁王、庆藩、韩藩、平凉支郡王合计折银,见 `content/regions.json` 陕西 `settle._meta.royal_stipends_source` | **4.07/月** |
| `Due.官俸` | 8/月 | 文武官俸卷(Src[4]) | **~3/月** |
| `逋赋率` | 0.3 | 陕西连年灾、欠赋高 | **0.45**(灾区上调) |
| `火耗率` | 0.2 | 火耗常 10–20% | 0.18(微调) |
| `官民田`/`隐田` | 3050/1600 万亩 | 《万历会计录》卷九田土官民共292,920.151顷;隐田无一手源(定义使然)沿用 #266 比率1600/3050推算 | **2929.20151 / 1536.6303 万亩**;`官民田` 已从 provisional 移除、`隐田` 仍 provisional |

> **三饷分量分时间线注入(非 seed 常量)**:开局 1627.10(turn=1)`三饷应征`=仅辽饷九厘 **2.1969011325/月**;到点事件改 `三饷应征`(=「饷率 effect 通道」)——崇祯四年1631 辽饷升一分二厘(=九厘值 4/3 倍)· 崇祯十年1637 剿饷开征(+陕西分量,崇祯十三1640 撤)· 崇祯十二年1639 练饷开征(+陕西分量)。各事件目标值按 `_meta` 基线与在征集重算。**起运定额须与三饷联动**:模型单一 `起运定额` 把正赋+三饷起运合并封顶、无法单独表达「三饷全解、正赋多留」——故三饷事件抬 `三饷应征` 时必同步抬 `起运定额=正赋起运基线0+Σ在征三饷`(≥当期三饷),否则超额三饷漏进省内池;step2 港口考量:或令 `起运定额` 跟随三饷动态、或拆三饷-起运独立通道(gemini PR#113;选型留可扩展空间,见 §port TODO 末「扩展性护栏」)。**→ 选型已定(#259 grill 2026-06-20):联动单定额(不拆),见下 §#259 / [ADR 0020](adr/0020-fiscal-levy-effect-channel-event-driven.md)。**
...
> **#585 收尾状态**:全 15 布政司首 tick golden 末态已按重标值重算并落 `tests/test_fiscal_substrate_bridge.py::test_all_15_regular_provinces_first_tick_golden`；比较统一用 `math.isclose(..., abs_tol=1e-3)`，显式保留非 0 绝对容差，避免省库库银/欠账近零时退化成纯相对比较。

> **#70/#585 shadow 阶段 riders(grill 定)**:无 hub 时各省 `起运到京` 只算/打印、不汇国库(hub 是单独项,ADR 0019);`拨付gross`(京运补)按史实**静态 seed**(边镇有/内地 0),hub 项以后才让它动态化;全 15 布政司 + 辽东 + 皮岛/东江均在 shadow/cutover tick,tlog 出史实量级(江南盈余 / 边镇陕晋辽赤字死亡螺旋 / 藩王重省宗禄拖累)。边镇「收入<义务→死亡螺旋」是**有意求生压力**(非配平 bug)。
...
- 绝对容差(EPS=1e-6/末态1e-3)按史实量纲重标后会失配；当前首 tick golden 已按上文「#585 收尾状态」改用显式 `math.isclose(..., abs_tol≠0)`。
...
>
> **升降通吃 + ADR 0014 事件链门(D3,读事件账)**:议停剿饷触发门 = **`剿饷开征.结局==已准`**(+天定时窗,纯 ADR 0014 `event.剿饷开征.terminal_reason` 寻址,**不读 settle.p 活态**——cmr-β)。**门里不加 `¬剿饷议停已发` 自指**(线上 codex:event.<id>.* gate-key 当依赖、自指成环→SettlementAbort);不重弹靠 ADR 0014 内建终态 skip。剿饷开征未达已准 → 议停按级联作废、不空弹。准奏议停 → 结局=已停 → 在征集去剿饷(在征谓词在**效果侧**读 剿饷议停.结局、非 gate)→ 分量回退、起运回落。
>
...
>
> **精度沿用 #70(D5)+ 万两国总口径(cmr-F)**:量级定稿、田亩占比折算、`_meta.provisional`;一手《会计录》首轮核精化已由 #585 收尾（全 15 布政司 + 辽东专属源 + 皮岛/东江维持无对应源口径）。加饷估算用**国总加征所得 vs 国总军费缺口**(全国一道旨意),非对单省缺口比。
>
> **落位契约(cmr R3 钉,设计级最小契约,确切键名/schema 留 /tdd)**:① 结局落 `event.<id>.terminal_reason` + 封闭标签集(开征/升率 {已准/已驳}、议停 {已停/仍征});议停门读 `剿饷开征.terminal_reason==已准`(**不自指 `剿饷议停.*`**、不重弹靠内建终态)、在征(效果侧)读 `剿饷议停.terminal_reason≠已停`,两谓词不同源;结局值经白名单校验/规范化、不 fallback、不可归一 fail-loud+retry(线上 gemini)。② 在征显式 per-饷:辽默认在征(辽饷升已准则升率)、练 iff 练饷开征已准、剿 iff 剿饷开征已准 ∧ 剿饷议停结局≠已停;**无议停事件/DB 查无记录 ⇒ 该项恒真**(防御:查无记录返「未停」、**不 raise**,线上 gemini;但仅对「定义存在、无触发记录」——按设计应有停链却**缺事件定义**则 **fail-loud**,线上 codex)。③ stub 同 tick 原子置结局(不留 outcomeless 中间 tick)+ 史实默认(开征/升率→已准、剿饷议停→已停)。④ 正赋起运基线存独立键(seed = **`max(0, seed起运定额−开局辽饷)`**,线上 gemini 防负值崩),通道写 `起运定额=正赋基线+Σ在征三饷`(≥Σ三饷、非负双保)、玩家偏移(#260)另算组合。⑤ applier 仅写 `{三饷应征,起运定额}`、保 `{正赋应征,Due,火耗率,逋赋率}` 不动。⑥ **pre_settle 顺序**:饷率结局置定 + applier **先于** `settle_province_tick`(同 tick)——**只前移饷率特化事件、不整体移 `apply_event_terminal_states`**(线上 gemini/CR:整体移会让其它依赖当月财政的事件读上月旧数据);现 `SETTLEMENT_FLOW` 序是 fiscal-then-events,#259 饷率特化前置(同月生效、不扰其它事件);饷率事件用 **category tag** 识别、不硬编 event ID(线上 gemini);前置饷率事件须在后置通用 pass **显式跳过防重**(线上 gemini)。**「同月生效」对「给定结局」成立(shadow/stub 路径);真玩家决策时序 = #260 seam**(事件在 pre_settle 内生成、晚于拟旨,#260 须自定「拟旨前暴露饷率议题」或接受玩家次月决,#259 标依赖不替拍——线上 codex P1)。
>
...
## 待精验
征收能力/士绅协作/粮价/运输/宗室口/驿卒 seed · 负担标准值校准。全 15 布政司的一手核首 tick golden 与 tlog smoke 已由 #585 收口；后续精验不再阻塞当前 substrate 收尾。
...
- **当前 provisional 口径（#585 收尾）**：宗禄已按卷三十二映射到省级 `Due.宗禄`，不再作为通用 provisional；`隐田`因定义上缺一手实数，15 布政司仍保留 provisional。山东卷六田赋/起运缺口仍标 `官民田`/`正赋应征`/`起运定额` provisional；南直隶已按卷十六移除 `官民田`/`正赋应征`/`起运定额` provisional。辽东按专属军饷源，皮岛/东江标记 `no_wanli_accounting_record` 并维持现状。
