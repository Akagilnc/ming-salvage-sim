...
**饷率 effect 通道 #259 — grill 收敛(2026-06-20)+ 设计 cmr R1 折叠(2026-06-21,引入饷别决策态),见 [ADR 0020](adr/0020-fiscal-levy-effect-channel-event-driven.md):**

> **定调更正(D1)**:三饷**不是**「到点强制改 `三饷应征`」的确定性背景,而是史实里皇帝的决策——加则民变深、不加则边军溃。本通道 = 史实时点**生成「军费危机 → 是否加这道饷」的历史事件**(ADR 0014 触发门)摆两难 + 标记该饷**待决**;民变/加征反噬交 LLM 软判(ADR 0007/0011)。**效果不在「触发即落」**(见下)。
>
> **饷别决策态(cmr R1 核心)**:每省、每饷分量(辽/剿/练)一个**持久状态 {未决/已准/已驳/已停}**——ADR 0014 之外的**第三类事件形态**(原只「软判 / 触发即落确定性后果」两类;本通道是「触发 → 待决 → 决断后落」)。效果**读决策态、按增量改 `settle.p`**,不触发即落;**状态转移由 #260 圣旨驱动**(#259 定义状态 + 读态效果、不实现圣旨路由 = #150/#260);**shadow 用显式 stub 置默认(史实 = 已准)、明标「#260 替换」** → live 不强改三饷(只读态)、shadow 默认是**可替换占位**(消解 cmr-D「shadow 自动应用 = 复活被否决的强制注入」)。
>
> **触发门 = 天定(D4b)**:军费窟窿如期出现,皇帝杠杆是「加饷 vs 另寻财源/硬扛」;**破局在上行软判(追赃/挪借/招抚/清丈),不在让战争消失**(越界战略线)。
>
> **升降通吃 + 决策态链门(D3,cmr-B 修)**:既生成开征/升率(辽饷 1631 升 / 剿饷 1637 开 / 练饷 1639 行)也生成议停(剿饷 1640 撤);**议停剿饷触发门 = 读「剿饷决策态 == 已准」**(持久决策态,**非活财政软读**——避开 ADR 0014 链门只认冻结账、表达不了「仍在征」的洞)。未准/已驳/已停则议停不触发。准奏议停 → 决策态转**已停**(终态、防重弹)、分量增量回退、起运回落。
>
> **起运增量联动 + 保正赋基线(D2,cmr-C/E 修)**:`起运定额 = 正赋起运基线 + Σ(active 三饷分量)`,正赋基线 seed 定、存 `_meta`;效果对三饷侧**按增量**改(**非绝对设**,故玩家缓起运/截留与饷率增量共存);**不变式 `起运定额 ≥ 正赋起运基线 + 当期三饷应征`**(双下界:守正赋解京 + 三饷全解)。**不拆三饷-正赋对外字段**(只有 #261 hub 才消费、违 boundary),内部留正赋基线供回落;#260 真要演「截留正赋」再拆对外字段。
>
> **失地/收复(cmr-G)**:饷率更新应用**全 17 seed 省 `settle.p`、不论当前控制方**;动态成员谓词只管「是否 settle/tick」、**不管「是否收 rate 更新」** → 失地省 `settle.p` 仍随时间线更新、收复即现行饷率(与 #265 on_restore 邻接)。
>
> **精度沿用 #70(D5)+ 万两国总口径(cmr-F)**:量级定稿、田亩占比折算、`_meta.provisional`、一手《会计录》deferred;月份锚正月 provisional;全 17 省经动态谓词。**加饷估算用国总加征所得 vs 国总军费缺口**(全国一道旨意),非对单省缺口比。
>
> **boundary**:#259 全程 shadow(起运到京只算/打印 = hub #261);**决策态的转移逻辑(圣旨 → 决策态)归 #260/#150**,#259 只定义状态 + 读态确定性效果;不实现加征反噬消费(ADR 0011)。详设(各饷各省目标值、事件文案、决策态字段落位、turn 锚点)进 #259 PRD。
