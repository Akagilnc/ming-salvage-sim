...
- **三饷按时间线拆**:辽饷(万历末起,崇祯四年增)/剿饷(崇祯十年,1637)/练饷(**崇祯十二年,1639** —— 杨嗣昌十一年议、十二年行;原误作十一年),seed 不能把三者都当开局常量;字段可叫 `三饷应征` 但分量分时间线注入。**开局 1628(崇祯元年正月,与游戏起手一致)只该有辽饷**(剿饷1637/练饷1639 尚未开征)。
...
> **三饷分量分时间线注入(非 seed 常量)**:开局 1628(崇祯元年)`三饷应征`=仅辽饷九厘 ~2.5/月;到点事件改 `三饷应征`(=「饷率 effect 通道」)——崇祯四年1631 辽饷升一分二厘(~3.3/月)· 崇祯十年1637 剿饷开征(+陕西分量,崇祯十三1640 撤)· 崇祯十二年1639 练饷开征(+陕西分量)。各事件目标值待 step2 编码时按陕西亩×率定。
