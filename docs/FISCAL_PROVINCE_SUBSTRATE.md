...
| `三饷应征`(辽饷) | 10/月 | 陕西亩 × 0.009→0.012(崇祯四年1631 升,Src[3]) | **~2.5/月**(~30万/年) |
...
> **三饷分量分时间线注入(非 seed 常量)**:开局 1628 `三饷应征`=仅辽饷(~2.5/月);剿饷(1637)/练饷(1639)做成到点触发的历史事件改 `三饷应征`(=「饷率 effect 通道」)。
> **重标后必做**:golden 末态硬期望全部按新数手推重算(确定性,spike 重跑自洽);EPS 绝对容差(1e-6/末态1e-3)随量纲变会失配,改相对容差。
