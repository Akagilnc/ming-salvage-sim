...
- **当前 provisional 口径（#585 收尾）**：宗禄已按卷三十二映射到省级 `Due.宗禄`，不再作为通用 provisional；`隐田`因定义上缺一手实数，15 布政司仍保留 provisional。山东卷六田赋/起运缺口仍标 `官民田`/`正赋应征`/`起运定额` provisional；南直隶 `起运定额` 仍 provisional。辽东按专属军饷源，皮岛/东江标记 `no_wanli_accounting_record` 并维持现状。
