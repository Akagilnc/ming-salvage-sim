...
- 绝对容差(EPS=1e-6/末态1e-3)按史实量纲重标后会失配,port 改 **scaled-abs 或 `math.isclose`(abs+rel 混合,须显式 `abs_tol≠0`——默认 0 近零仍退化纯相对)**——**不可纯相对**(省库库银死亡螺旋常 →0,近零纯相对失效;同上「重标后必做」注)。
...
- **[5] 官俸薄**（参考）：[北京日报·明代官俸](https://news.bjd.com.cn/2022/08/15/10134150.shtml)
