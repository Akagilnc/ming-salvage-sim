...
| `起运定额`(=起运**到京/送京师**上限,非起运-边军) | 40/月 | 陕西=赤字边镇,田赋多供本镇边军(settle_tick 记省内/存留)+倚京运补、送京师少。⚠️会计录「起运」类含起运-边军(≠模型的送京),其陕西起运/存留比待一手核 | **~3/月**(送京师少;若会计录证陕西到京起运高比例,step2 上调——逐条可驳) |
...
> **三饷分量分时间线注入(非 seed 常量)**:开局 1627.10(turn=1)`三饷应征`=仅辽饷九厘 ~2.5/月;到点事件改 `三饷应征`(=「饷率 effect 通道」)——崇祯四年1631 辽饷升一分二厘(~3.3/月)· 崇祯十年1637 剿饷开征(+陕西分量,崇祯十三1640 撤)· 崇祯十二年1639 练饷开征(+陕西分量)。各事件目标值待 step2 编码时按陕西亩×率定。
...
> **重标后必做**:golden 末态硬期望全部按新数手推重算(确定性,spike 重跑自洽);EPS 容差(绝对 1e-6/末态1e-3)随量纲缩小会失配——**改 scaled-abs 或 `math.isclose`(abs+rel 混合),不可纯相对容差**(省库库银死亡螺旋常 →0,纯相对容差近零失效:0±rel 恒 0、比不出真差)。
...
- 绝对容差(EPS=1e-6/末态1e-3)按史实量纲重标后会失配,port 改 **scaled-abs 或 `math.isclose`(abs+rel 混合)**——**不可纯相对**(省库库银死亡螺旋常 →0,近零纯相对失效;同上「重标后必做」注)。
