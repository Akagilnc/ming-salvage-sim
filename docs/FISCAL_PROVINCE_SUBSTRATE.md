...
**饷率 effect 通道 #259 — grill 收敛(2026-06-20,见 [ADR 0020](adr/0020-fiscal-levy-effect-channel-event-driven.md)):**

> **定调更正(D1)**:三饷**不是**「到点强制改 `三饷应征`」的确定性背景,而是史实里皇帝的决策——加则民变深、不加则边军溃,两边有代价。本通道 = 史实时点**生成「军费危机 → 是否加这道饷」的历史事件**(ADR 0014 触发门)把两难摆到皇帝面前;**加/不加走全局圣旨通道(#260)**、复用 ADR 0014 + ADR 0013 决策落库;**准奏后**本通道落确定性后果(`三饷应征` 升/降 → v23 火耗联动 → 起运联动);民变/加征反噬交 LLM 软判(ADR 0007/0011)。本通道不造皇帝决策机、不直接强改三饷。
>
> **触发门 = 天定(D4b)**:辽东/流寇/练兵军费窟窿如期出现(根因不在 #259 范围),皇帝杠杆是「加饷 vs 另寻财源/硬扛民变」;**破局在上行软判(追赃/挪借/招抚/清丈),不在让战争消失**(越界战略线)。守「压力源 + 破局可真」。
>
> **升降通吃 + 事件链门(D3)**:既生成开征/升率(辽饷 1631 升 / 剿饷 1637 开 / 练饷 1639 行)也生成议停(剿饷 1640 撤);**议停触发门前置「该饷已开征且仍在征」**——皇帝当年没加征则议停避过(ADR 0014 链门、不空弹)。准奏议停则确定性减分量、起运定额回落。
>
> **起运联动 = 单定额(D2,定 gemini PR#113 选型)**:三饷事件升/降 `三饷应征` 时同步升/降**同一个 `起运定额`**(≥当期三饷,保证全解);**不拆三饷-正赋独立通道**(拆出字段只有跨省 hub #261 才消费、违 boundary 铁律);字段留可扩展口,将来 #260 要演「截留正赋」再拆。**不变式断言 `起运定额 ≥ 当期三饷应征`**(漏做则超额三饷静默漏进省内池、`settle_tick` 不报错)。
>
> **精度沿用 #70(D5)**:量级定稿、田亩占比折算(剿/练饷按国总 × 省田亩占比、辽饷按陕西锚 × 率)、`_meta.provisional`、一手《会计录》deferred;各饷开征/升率/停征「月份」锚该年正月(provisional、`_meta` 标注,一手核诏令月 deferred)。覆盖全 17 省经动态成员谓词(#70 seed 后),非单省。
>
> **boundary**:#259 全程 shadow(起运到京只算/打印、不汇国库 = hub #261);不实现圣旨 → action 拆分(#150/#260)、不实现加征反噬消费(ADR 0011)。= 生成事件 + 落准奏后确定性后果 + 保持辽/剿/练分量可归因,止于此。D3/D4b/D5 的详设(各饷各省目标值、事件文案、触发门谓词、turn 锚点)进 #259 PRD(to-prd)。
