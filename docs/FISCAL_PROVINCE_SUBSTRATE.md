...
>
> **落位契约(cmr R3 钉,设计级最小契约,确切键名/schema 留 /tdd)**:① 结局落 `event.<id>.terminal_reason` + 封闭标签集(开征/升率 {已准/已驳}、议停 {已停/仍征});议停门读 `剿饷开征.terminal_reason==已准`(**不自指 `剿饷议停.*`**、不重弹靠内建终态)、在征(效果侧)读 `剿饷议停.terminal_reason≠已停`,两谓词不同源。② 在征显式 per-饷:辽默认在征(辽饷升已准则升率)、练 iff 练饷开征已准、剿 iff 剿饷开征已准 ∧ 剿饷议停结局≠已停;**无议停事件/DB 查无记录 ⇒ 该项恒真**(防御性:查无记录返「未停」默认、**不 raise**,线上 gemini)。③ stub 同 tick 原子置结局(不留 outcomeless 中间 tick)+ 史实默认(开征/升率→已准、剿饷议停→已停)。④ 正赋起运基线存独立键(seed = **`max(0, seed起运定额−开局辽饷)`**,线上 gemini 防负值崩),通道写 `起运定额=正赋基线+Σ在征三饷`(≥Σ三饷、非负双保)、玩家偏移(#260)另算组合。⑤ applier 仅写 `{三饷应征,起运定额}`、保 `{正赋应征,Due,火耗率,逋赋率}` 不动。⑥ **pre_settle 顺序**:饷率结局置定 + applier **先于** `settle_province_tick`(同 tick)——**只前移饷率特化事件、不整体移 `apply_event_terminal_states`**(线上 gemini/CR:整体移会让其它依赖当月财政的事件读上月旧数据);现 `SETTLEMENT_FLOW` 序是 fiscal-then-events,#259 饷率特化前置(同月生效、不扰其它事件)。
>
