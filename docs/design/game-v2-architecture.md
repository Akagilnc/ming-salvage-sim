...
**2026-09-10 释义（给事中审切片票上呈后 owner 逐字）**：「是这样的。llm可以提。但是引擎有上限，llm可以说我抄家魏忠贤拿了1亿银子。单引擎能给出来的就是魏忠贤所有的可能五百万两。别的数字也是一样」——「引擎计算的活」= 引擎按账本实有封顶并双边扣账，LLM 提名义数；抄没封顶需人物家产存量账（现无，切片 C5）。#1820 决定不变。
