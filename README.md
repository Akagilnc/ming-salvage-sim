...
本仓库是基于 [wangwei-ying3/ming-salvage-sim](https://github.com/wangwei-ying3/ming-salvage-sim) 的微硬核分支。

上游已经搭好了晚明对话式政略模拟的基本框架：月度回合、大臣召见、诏令结算、月末邸报、人物记忆、地区局势和外部势力。本分支继续往一个更“硬”的方向推进：

> **AI 负责奏对、推演和人心；引擎负责账本、状态和后果。**

也就是说，故事可以由 AI 来讲，局势可以由 AI 来判断，大臣可以推诿、敷衍、站队、误导；但关键结果不能只停在漂亮邸报里。
...
| 方向 | 上游重点 | 本分支重点 |
| --- | --- | --- |
| 玩法核心 | 对话式晚明政略模拟 | 对话式政略 + 状态可信 + 后果落库 |
| AI 角色 | 生成奏对、邸报、推演 | 继续负责人心和判断，但不独占事实 |
| 结算 | 月末根据诏令推进局势 | 更强调结构化结果、账本、事务边界和可复盘 |
| 历史事件 | 按时间与局势推进 | 逐步加入结构化前提门，避免“玩家干预无效” |
| 财政 | 国库、内库、省级税收等基础模型 | 往三饷、火耗、逋赋、起运、欠饷、宗禄等真钱账本深化 |
| 开发目标 | 可长期游玩的晚明皇帝模拟器 | 可长期游玩，也可作为 LLM 历史模拟实验台 |

一句话：

> 上游解决“这个游戏能不能成立”；本分支解决“这个游戏能不能更可信地跑下去”。
...
- **结算事务边界**：避免中途失败后留下半落库状态。
- **省级财政基座**：从单省财政 spine 开始，把三饷、火耗、起运、欠饷、宗禄等做成可检验账本。
- **fail-loud**：畸形数据、未知字段、落库失败不能静默吞掉。
...
- [TODO / TOFIX](docs/TODO.md)

---

## 致谢

本分支基于 [wangwei-ying3/ming-salvage-sim](https://github.com/wangwei-ying3/ming-salvage-sim)。

原项目已经完成了最重要的一步：证明“晚明皇帝模拟器 + LLM 对话 + 月度政略结算”这条路是可走的。本分支在此基础上继续推进状态可信、财政基座、结构化落库和长期跑局稳定性。
