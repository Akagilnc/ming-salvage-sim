...
- **省级财政基座**：明控且已 seed 的省会在月末推进账本，把三饷、火耗、起运、欠饷、宗禄等做成可检验状态；军饷欠账已按省/中央来源接入 per-source 容器，起运入国库的跨省 hub 仍是后续工作。
...
- 更稳定的历史事件硬门。
- 新手引导和前端体验。
- 财政基座的跨省 hub、国库 cutover 与长线平衡调参。
...
| 经济系统 | 国库 / 内库 / 地方税收 | 现有模型已能结算收支；省级财政基座推进已 seed 省份并接入军饷欠账容器，起运入国库 hub 尚未切换。 |
...
- 推进省级财政基座的跨省 hub、国库 cutover 和旧财政路径并轨。
