...
**出题资格**：只在盘面真实出现亲裁级抉择时出题；盘面平稳则直接收束奏章、整段不出决策块。抉择只就盘面已有的张力设问。`context` 措辞按分量分层，重者写「非亲断不可」，中等分量如实写「臣不敢擅专，奏请圣裁」，分量轻则照实写。
...
- 决策块只放奏章**最末尾**，放在「诏书核销」之后；块外只收束。盘面平稳、确无亲裁级抉择时，奏章在核销后直接收束，整段决策块不出。
