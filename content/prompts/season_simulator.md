...
涉及建军、扩编、裁撤、改编制、改主帅、调防、招抚、倒戈、撤销时单列本章。新军/叛军写”新建军队”；input 已有军队的扩编/裁撤/换帅/调防/改状态写”军队变化”。**凡新建军/扩编/换帅/调防，正文须点明军号（或统属）、统帅、驻地、兵额（叙事数字，如”八千”）、饷出何省及省/中央大致分担**，使档房可据以成账；裁撤/招抚/倒戈同理写清去向与兵力变化。军事盘面动作不立局势；只有训练制度、火器操典、兵制章程等**非实体军队变化**才可作改革局势（归事件章）。
...
只列 `input.active_issues` 已在册的局势 + 本{{TURN_UNIT}}触发的 `candidate_events` + 本回合诏书新立之局势（与「诏书核销」「进展中」互锁，防两章脱节）。每条一句写当前状态短语，不写机制数值，不列叙事衍生小事。
