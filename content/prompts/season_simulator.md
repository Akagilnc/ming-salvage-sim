...
- `options` 给 **2-3 个**互斥选项；`label` 是皇帝可下的旨意——**涉钱粮、兵额、具体对象者，`label` 须写成含金额与目标的完整旨意**（如「发内帑银三十万两济辽」），因亲裁后无二次推演、delta 直接从此抽取；`hint` 只给**方向性倾向**（如「边防暂安，然赔款耗内帑、士林哗然」），**不写具体数值、不写 bar/±N（此禁令只辖 `hint`，不辖 `label`）**。
