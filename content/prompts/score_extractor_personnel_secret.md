...
- 字段：`name`、`动作`、`office`、`reason`，可选 `faction`、`office_type`、`region_id`/`任所`；所有字段均为字符串。
...
- 地方/督抚/边镇任官必须显式填 `region_id`（或 `任所`）：值为 input `region_ids` 内英文 id（如 `fujian`、`shaanxi`）。这是任所辖域，不是 `行止.transit_to` 物理去向；缺任所的地方任命会被逐项拒收。中央衙门任命不填。
