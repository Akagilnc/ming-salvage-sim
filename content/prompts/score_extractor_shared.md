...
1. **逐句扫邸报，给每句贴标签**：这句是「本{{TURN_UNIT}}新动作」还是「旧案复述／在途续办／历史回顾／传闻未证」？只有新动作才可能落账；旧案、在途、回顾、传闻一律不落。同时对照 `previous_narrative`／`relevant_memories`，凡上{{TURN_UNIT}}已落过账的事（已拨、已抄、已抓、已开工、已册封）本{{TURN_UNIT}}再提即续办，不重复落。
...
| `钱粮收支` | array，每项 `{"账户":"国库\|内库","增量":整数,"分类":"...","原因":"...","来源引用":"dossier:<id>\|盘面自发"}`；无案卷后果可改写同一 typed `事务声明`，此时来源引用可空 | 一次性收支，单位万两；增量正入负出。两钱库互拨仅写来源负项，加 `transfer_to` 指明目标库，由引擎双边入同一实数 |
...
| `人口转移` | array，每项 `{"源":"农民@shaanxi","目标":"流民@shaanxi","数额":整数,"原因":"加派\|摊派\|灾害\|兵灾\|逃亡","来源引用":"dossier:<id>\|盘面自发"}`；`源`/`目标` 必须同省省级行 | 人口守恒转移（#649/0087）：单条记录同事务源阶级减 N、目标阶级增 N；方向矩阵——加派/摊派/灾害＝农民→流民、兵灾＝农民/军户→流民、逃亡＝军户→流民；`数额` 是拟转正整数，单位随 input `population_unit`，勿混刻度，applier 按当时源阶级省级行实有封顶并将同一实际量写入两侧（各省人口余额见 input `class_population_balances`）；灾害/兵灾须有该省盘面事实支撑（灾害吃 region 天灾字段或灾情/饥荒类局势 issue，兵灾吃战事过境/围城等兵祸事实或军事高压档），无此事实不得申报；灾害/兵灾的具体拟转量依据灾情/兵情、input `class_population_balances` 与 `population_unit` 软判，不设固定比例或累计 cap。**`回流` 不由 extractor 申报**——赈济/招抚屯田由引擎 recovery 单核唯一写入（#652）。流民投贼走军务档 `流民投贼`，不走本字段 |
...
| `人物变更` | array，每项必须含 name、动作，以及 `来源引用` 或无案卷时的同一 typed `事务声明`；`动作` 只能是：{{PERSON_ACTIONS}}；按动作补 `office`/`office_type`/`region_id`/`status`/`new_power`/`transit_to`/`loyalty`/`style`/`reason` | ADR0009 人事档案单一入口；地方/督抚/边镇任命须显式 `region_id`/`任所`（input `region_ids` 英文 id，任所≠行止）；`行止` 只填来自 `region_ids` 的非空 `transit_to`，不得填 `location`；`评定.loyalty` 是忠诚增量；`性情.style` 是人物固有层完整替换 |
| `密令副作用` | array，每项 `{"密令编号":整数,"推演备注":"...","泄漏结论":boolean}`（泄漏结论可省略） | `在办` 组密令副作用；情节已实际公开（disclosed）才为 true |
| `密令执行态` | array，每项 `{"密令编号":整数,"执行态":"忠实\|打折\|阳奉阴违\|反噬","备注":"..."}` | 在办密令由本档房判断执行态；结案不由本字段 |
