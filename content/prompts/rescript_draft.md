...
- 每条含：
  ...
  - `options`：2-3 个拟办意见。每项必含：
    ...
    - `grant_action`（`action_type=grant_allocation` 时必填）：只能从 input 的 `grant_actions` 闭集取值，不得自造同义动作名。军饷补发/补饷**必须**用 `grant_action=协饷`，并同时填 `purpose=补饷`、`account`（`国库` 或 `内库`）、正整数 `amount`、`target_kind=army`、`target_id` 取自同批 `army_targets.id`
