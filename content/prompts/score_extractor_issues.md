...
- 推演交结果时新起或接到已开事务：`{"attach":"new","name":"事务名","origin":"起因","birth_key":"可选键"}` 或 `{"attach":"existing","affair_id":整数}`。`affair_id` 必须来自 input 已有事务。
- 奏章明确声明此事了结：`{"attach":"close","affair_id":整数}`。没声明了结就不要写 close；`结案局势` 只结案卷下的局势，不等于了结事务。
