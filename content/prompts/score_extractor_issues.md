...
- **第 2 步（列候选）**：只列 `局势推进`/`新立局势`/`撤销局势`/`结案局势`/`事务声明`，其余字段不碰。军队实体变化归军务档房，不在这里立 issue。事务了结只写 `事务声明` 的 `attach=close`，不得用 `结案局势` 代替，也不得从散文猜。
...
月末奏章若出现朝臣弹劾/检举他人差务，输出 `政敌检举` 数组。每项：`{"检举人":"规范名","被检举人":"规范名","所指案卷":12,"弹章正文":"奏疏原文摘要"}`。
- `所指案卷` 必须是真实存在的案卷 id（来自 `decree_dossiers` / 事实包 `forked_dossiers`）；对不上真实案卷的条目引擎会拒。
- `检举人` 必须是在朝人物规范名。
...
若奏章明确列出差事承办/参与人物，`新立局势` 必须同时输出结构化 `participants`（或等价的
...
- 推演交结果时新起或接到已开事务，把同一 typed 对象写在所声明的结果上（`新立局势`、`钱粮收支`、`人物变更` 等同形 `事务声明`）：`{"attach":"new","name":"事务名","origin":"起因"}`（同一批多项同属一件新事务时加同一 `identity`；无 `identity` 则各自独立，不得靠同名同起因合并）或 `{"attach":"existing","affair_id":整数}`（`affair_id` 来自 input `open_affairs`）。不得只在顶层数组创建或查找一个没有后果指向的空事务。
