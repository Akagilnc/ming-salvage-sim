...
3. `candidate_events` 中 `origin_kind:"impeachment_surge"` 的动态发难候选：发难派系角色依据 `faction_persona` 自主决定是否发难；不输出即不发难。发难时只输出 `origin_kind:"impeachment_surge"`、`candidate_id`（候选 `id`）、`faction_hint`（候选 `faction_id`）、`participant_roster`、`title`、`stage_text`；`participant_roster` 中的标靶必须来自候选 `eligible_target_ids`。`title` 与 `stage_text` 由角色自由生成，不复写候选事实字段。
