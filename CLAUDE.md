...
- **Slice-end review of the diff** → the per-slice review is the **whole-slice coder worker's OWN** job (ADR 0026 consolidate): the coder (Sonnet) builds the slice AND reviews it to convergence INSIDE its one memory-bearing session — there is **no separate reviewer worker**. Per-slice review is **TWO steps, both single-vendor — the per-slice cmr DEGRADED to Sonnet-writes / Opus-single-leg-reviews** (the full cross-model cmr is family-only): **(1)** the builtin **`/review`** over the slice diff (the coder runs it, fixes findings, does the self-check 二连); then **(2)** a fresh single **Opus subagent** (`Agent`/`Task`, `model: opus`) that 评审 the diff — fix its findings (route a non-trivial fix through `diagnosing-bugs`) + self-check 二连, commit, then re-dispatch a FRESH Opus subagent, looping to convergence (no blocking findings). **NEVER run `ak-cross-m-review` (codex+agy+claude) per-slice; NEVER spawn codex/agy legs per-slice.** The full cross-model `ak-cross-m-review` is the **separate integrated-cmr worker** the runner runs ONCE over the assembled family base (the cross-family 承重闸, ADR 0022 / 0026 — catches the 跨片接缝 a per-slice review cannot see; fans out N codex + agy, +1 Claude `Agent` leg at ship-pre; needs the baked codex/agy CLIs + auth, internally invokes `diagnosing-bugs`). Decomposition: **per-slice = builtin `/review` + one Opus subagent 评审 (both inside the coder session); integrated = `ak-cross-m-review` (distinct family-layer worker, full codex+agy+claude)**.
