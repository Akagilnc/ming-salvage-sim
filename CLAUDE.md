...
- **Slice-end review of the diff** → this is the **reviewer worker's** job (a separate worker, ADR 0026 / PRD #330 R2), NOT the coder worker's. A coder/fix worker does NOT self-review at slice end — it commits and reports; the runner dispatches a fresh reviewer worker. The reviewer runs **two passes, both** (they stack, not replace): the builtin `/review` command (single-vendor) **and** the `ak-cross-m-review` skill (cross-model — fans out N codex + agy, +1 Claude `Agent` leg at ship-pre; needs the baked codex/agy CLIs + auth). Both ride the 2b full-pack image (this image). `ak-cross-m-review` internally invokes `diagnosing-bugs` on a non-trivial finding — that chain is present.
