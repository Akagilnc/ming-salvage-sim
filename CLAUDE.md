...
- **Slice-end review of the diff** → per-slice review is runner-dispatched and role-separated (ADR 0030): **coder** implements and commits；**reviewer** 是 fresh / read-only worker，评当前 full diff 并自报 open-count；**coder-fix** 逐项修复或证伪并执行自查二连。若 repo 有变化，由 Change Finalization 创建新的 non-amend commit；若全部证伪且无变化，合法 no-op。Runner 随后固定派 fresh reviewer 全量复审。Loop 直到 reviewer 自报 0 或 worker 主动提交 decision gate。**Do not run `ak-cross-m-review` per-slice;** full cross-model `ak-cross-m-review` remains the separate integrated CMR worker over the assembled family base.
