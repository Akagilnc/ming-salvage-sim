...
- **Slice-end review of the diff** → this is the **per-slice reviewer worker's** job (a separate worker, ADR 0026 / PRD #330 R2), NOT the coder worker's. A coder/fix worker does NOT self-review at slice end — it commits and reports; the runner dispatches a fresh reviewer worker that runs **ONE `/review` pass** (the builtin `/review` command) over the slice diff. That is the whole per-slice review — a single-vendor pass inside the fix-loop. The cross-model `ak-cross-m-review` (cmr) is NOT a second per-slice pass: it is the **separate integrated-cmr worker** the runner runs ONCE over the assembled family base (the cross-family 承重闸, ADR 0022 / 0026 — it catches the 跨片接缝 a per-slice review cannot see; fans out N codex + agy, +1 Claude `Agent` leg at ship-pre; needs the baked codex/agy CLIs + auth, and internally invokes `diagnosing-bugs` on a non-trivial finding). So the decomposition is **per-slice = `/review` (inner fix-loop reviewer), integrated = `ak-cross-m-review` (distinct family-layer worker)** — both ride the 2b full-pack image, but they are different worker steps the runner schedules at different points, not two stacked passes in one per-slice reviewer.
