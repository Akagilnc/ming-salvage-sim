...
- **Slice-end review of the diff** → **reviewer** 是 fresh / read-only worker，只评当前 full diff 并自报 open-count；**coder-fix** 逐项修复或证伪并执行自查二连；Change Finalization 自行处理 non-amend commit 或合法 no-op。每个 worker 完成本角色即停，准确接力只读 #869。**Do not run `ak-cross-m-review` per-slice;** full cross-model `ak-cross-m-review` remains the separate integrated CMR worker over the assembled family base.
