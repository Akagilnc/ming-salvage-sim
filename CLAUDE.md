...
- **Slice-end review of the diff** → the per-slice review is the **whole-slice coder worker's OWN** job (ADR 0026 consolidate): the coder builds the slice AND reviews it to convergence INSIDE its one memory-bearing session — there is **no separate reviewer worker**. Per-slice review is **TWO steps, both inside the coder worker** (the full cross-model cmr is family-only): **(1)** the builtin/equivalent **`/review`** over the slice diff (the coder runs it, fixes findings, does the self-check 二连); then **(2)** in the current **Claude-paused local pipeline**, a fresh single **non-Claude reviewer leg** over the current full diff (Codex by default; agy may substitute if Codex is unavailable). Fix findings (route a non-trivial fix through `diagnosing-bugs`) + self-check 二连, commit, then re-dispatch a FRESH non-Claude reviewer leg, looping to convergence (no blocking findings). **Do not call Claude/Anthropic locally for this per-slice leg while the Claude-paused pipeline is active. NEVER run `ak-cross-m-review` per-slice; NEVER spawn a full reviewer matrix per-slice.** The full cross-model `ak-cross-m-review` is the **separate integrated-cmr worker** the runner runs ONCE over the assembled family base (the cross-family 承重闸, ADR 0022 / 0026 — catches the 跨片接缝 a per-slice review cannot see; currently local Claude legs are paused, so this gate uses the configured codex/agy fallback and online bots remain final authority). Decomposition: **per-slice = coder review + one non-Claude reviewer leg (both inside the coder session); integrated = `ak-cross-m-review` (distinct family-layer worker)**.
