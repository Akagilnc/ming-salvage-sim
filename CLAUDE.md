...
6. **逐切片各开新 session `implement`**（canonical 构建步；自治/编排器里不直接 invoke umbrella，内联其流程）：约定 seam 调 `/tdd`（never refactor while RED）→ 跑 typecheck/单测/全量；**代码级实现现场长**（架构级早在 to-spec 钉了）。手动/单 session 流随后建 baseline commit；编排器 coder 停在 internal self-check，之后的 independent Verification → Change Finalization baseline 接力只读 #869。硬 bug → `diagnosing-bugs`、架构清理 → `improve-codebase-architecture`。
...
- **Implement a slice / build a feature / fix a bug test-first** → inline Matt `implement` flow rather than invoking the umbrella skill directly: invoke the `tdd` skill (Claude: `Skill` tool with skill `tdd`; Codex: load `~/.claude/skills/tdd/SKILL.md` and follow it as the active skill), drive red → green → refactor, then run typecheck / focused tests / full tests. Manual/single-session implement may then create its baseline commit before `/code-review` because that skill reviews `fixed-point...HEAD`, not uncommitted changes. In the orchestrator, **coder** 只完成 implementation / tests / internal self-check，尚不提交；independent Verification 收敛后，独立 Change Finalization Action 才创建 baseline commit。coder 不 fan out reviewer、不自行调用 `/code-review`；准确接力只读 #869。
