...
- **Implement a slice / build a feature / fix a bug test-first** → inline Matt `implement` flow rather than invoking the umbrella skill directly: invoke the `tdd` skill (Claude: `Skill` tool with skill `tdd`; Codex: load `~/.claude/skills/tdd/SKILL.md` and follow it as the active skill), drive red → green → refactor, then run typecheck / focused tests / full tests, then create a baseline commit. `implement` itself has `disable-model-invocation: true`, so worker prompts must spell out this inline sequence. Manual/single-session implement runs `/code-review` after the baseline commit because that skill reviews `fixed-point...HEAD`, not uncommitted changes. In the orchestrator, the **coder** stops at implementation/tests/self-check/commit; runner dispatches a separate reviewer worker for `/code-review`, and the reviewer soul supplies the fixed point (`origin/main` if available, otherwise `main`) so the worker never pauses to ask. Coder workers must not fan out reviewers or run `/code-review` themselves. Review fixes are new commits, never amend.
