...
6. **逐切片各开新 session `implement`**（canonical 构建步；自治/编排器里不直接 invoke umbrella，内联其流程）：约定 seam 调 `/tdd`（never refactor while RED）→ 跑 typecheck/单测/全量 → 调 `/code-review`（Standards + Spec 两轴单评；不能 fan-out 的 nested 场景才退内置 review 并显式标注）→ commit；**代码级实现现场长**（架构级早在 to-prd 钉了）。硬 bug → `diagnosing-bugs`、架构清理 → `improve-codebase-architecture`。
7. 〔项目加〕代码评审：per-slice `ak-cross-m-review`（**本项目手动多-session 开发流**的逐切片 cmr：codex+agy 跨家族——区别于 `## Skill routing` 里 #330 **编排器容器 worker** 的分解，那里 `/code-review` 是 Matt 单评、`ak-cross-m-review` 仍作整合 cmr）+ ship-pre 双闸 + 线上 bot；`gstack-ship` 收尾。
...
- **Implement a slice / build a feature / fix a bug test-first** → inline Matt `implement` flow rather than invoking the umbrella skill directly: invoke the `tdd` skill (Claude: `Skill` tool with skill `tdd`; Codex: load `~/.claude/skills/tdd/SKILL.md` and follow it as the active skill), drive red → green → refactor, then run typecheck / focused tests / full tests, then run `code-review` for the single Standards + Spec review before the baseline commit. `implement` itself has `disable-model-invocation: true`, so worker prompts must spell out this inline sequence. If the worker is in a nested context where `code-review` cannot fan out safely, use the built-in review fallback and report that downgrade.
