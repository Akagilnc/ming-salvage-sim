...
3. **`to-spec`**（旧名 to-prd，产物即 PRD）→ **完整 PRD**（不访谈，只综合 grill 的结论）：详尽 user stories + **Implementation Decisions + Testing Decisions（两层设计）** + Out of Scope；发 issue tracker 当父/epic、贴 `ready-for-agent`（父票贴标无所谓、不用管——admission 只读子票标签且有 parent_issue 双保险，本项目无扫标开工机制）。**禁文件路径/代码片段。**
...
5. 〔项目加〕设计评审（**在 to-tickets 之后**——cmr 审含切片布线的设计全家；#470/#471/#478 实践序，2026-07-03 修订原 4/5 对调）：`ak-cross-m-review`（本地 cmr）+ 线上 bot → 合 ADR、Status→Accepted。**评审态真源 = ADR Status**（owner 2026-07-14 简化）：Accepted 前=未过审，无论票上贴什么标；不搞撤标/贴回仪式——本项目无扫标开工机制，编排器只认你点火的家族。〈设计侧到此结束〉
6. **逐切片各开新 session `implement`**（canonical 构建步；自治/编排器里不直接 invoke umbrella，内联其流程）：约定 seam 调 `/tdd`（never refactor while RED）→ 跑 typecheck/单测/全量；**代码级实现现场长**（架构级早在 to-spec 钉了）。手动/单 session 流随后建 baseline commit；**#869 目标态**中编排器 coder 停在 internal self-check，之后由 independent Verification → Change Finalization 建 baseline commit；对应实现切片落地前，当前仍按现有 coder prompt 产出 commit。硬 bug → `diagnosing-bugs`、架构清理 → `improve-codebase-architecture`。
7. 〔项目加〕baseline commit 后代码评审：手动/单 session 场景单评用 `/code-review`（Standards + Spec 两轴；它评 `fixed-point...HEAD`，所以必须已有 commit）；编排器由 #869 在 Verification 收敛和独立 Change Finalization 后，由 runner 直接派 Standards-axis 与 Spec-axis 两条 fresh review leg，卷面交驻庭判官裁决；coder worker 不起 reviewer、不跑 `/code-review`。然后 per-slice `ak-cross-m-review`（**本项目手动多-session开发流**的逐切片 cmr：codex+agy 跨家族——区别于 `## Skill routing` 里 #330 **编排器容器 worker** 的上述双腿分解，`ak-cross-m-review` 仍作整合 cmr）+ ship-pre 双闸 + 线上 bot；`gstack-ship` 收尾。评审修复一律追加新 commit，禁止 amend。
...
> **ADR 0062 摘要**：编排器 Runner 只读三信号（exit code / judge typed tri-state / decision gate），不读卷面散文；`DELETE > patch` 是专业 reviewer / fixer 的 kill-axis，Runner 不读取或解释 finding 文本与 DELETE / patch 取舍。
...
- **Implement a slice / build a feature / fix a bug test-first** → inline Matt `implement` flow rather than invoking the umbrella skill directly: invoke the `tdd` skill (Claude: `Skill` tool with skill `tdd`; Codex: load `~/.claude/skills/tdd/SKILL.md` and follow it as the active skill), drive red → green → refactor, then run typecheck / focused tests / full tests. Manual/single-session implement may then create its baseline commit before `/code-review` because that skill reviews `fixed-point...HEAD`, not uncommitted changes. **#869 目标态**中编排器 coder 只完成 implementation / tests / internal self-check，independent Verification 收敛后由独立 Change Finalization Action 创建 baseline commit；对应实现切片落地前，当前仍按现有 coder prompt 产出 commit。coder 不 fan out reviewer、不自行调用 `/code-review`；准确接力只读 #869。
...
- **Slice-end review of the diff** → runner 直接派 Standards-axis 与 Spec-axis 两条 fresh / read-only leg，腿只产 raw prose 交驻庭判官裁决；**coder-fix** 逐项修复或证伪并执行自查二连；**Repair Change Finalization** 自行处理新的 non-amend commit 或合法 no-op。每个 worker 完成本角色即停，准确接力只读 #869。**Do not run `ak-cross-m-review` per-slice;** assembled delivery base 上只由两个相互独立、fresh-context 的 Integrated Completeness 与 Integrated Correctness reviewer Action 共用该 engine；准确顺序只读 #869。
- **Resolving an in-progress git merge/rebase conflict (family merge fallback)** → this is the **merger worker's** job (ADR 0022 / 0026): invoke the `resolving-merge-conflicts` skill and preserve BOTH sides' behaviour. Merge Action 拥有合并与冲突生命周期；merger 只解决当前真实冲突并返回，准确调用位置只读 #869。
