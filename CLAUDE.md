...
6. **逐切片各开新 session `implement`**（canonical 构建步；自治/编排器里不直接 invoke umbrella，内联其流程）：约定 seam 调 `/tdd`（never refactor while RED）→ 跑 typecheck/单测/全量 → 单评 → commit；**代码级实现现场长**（架构级早在 to-prd 钉了）。手动/单 session 场景单评用 `/code-review`（Standards + Spec 两轴）；**编排器 coder worker 不起 reviewer、不跑 `/code-review`**，单评由 runner 派出的 reviewer worker 承担。硬 bug → `diagnosing-bugs`、架构清理 → `improve-codebase-architecture`。
7. 〔项目加〕代码评审：per-slice `ak-cross-m-review`（**本项目手动多-session 开发流**的逐切片 cmr：codex+agy 跨家族——区别于 `## Skill routing` 里 #330 **编排器容器 worker** 的分解，那里 `/code-review` 是 runner 派发的 Matt 单评 reviewer worker、`ak-cross-m-review` 仍作整合 cmr）+ ship-pre 双闸 + 线上 bot；`gstack-ship` 收尾。
...
- **Implement a slice / build a feature / fix a bug test-first** → inline Matt `implement` flow rather than invoking the umbrella skill directly: invoke the `tdd` skill (Claude: `Skill` tool with skill `tdd`; Codex: load `~/.claude/skills/tdd/SKILL.md` and follow it as the active skill), drive red → green → refactor, then run typecheck / focused tests / full tests. `implement` itself has `disable-model-invocation: true`, so worker prompts must spell out this inline sequence. Manual/single-session implement then runs `code-review` before committing. In the orchestrator, the **coder** stops at implementation/tests/self-check/commit; runner dispatches a separate reviewer worker for `code-review` or the configured fallback. Coder workers must not fan out reviewers or run `code-review` themselves.
