...
| `parked: no usable completeness review legs` / `cross-vendor jury floor not met` | no transport-live legs (exit0 + non-empty stdout) and/or vendor diversity floor unmet — content shape is never a gate (ADR 0141 / #1005 landed) | don't blind-retry dead transports; hand-run the integrated CMR and answer the gate with its verdict |
