...
class ErrorEdgeBackend implements Backend {
...
describe("S8 tri-state: success / escalate / error are all distinct and caller-distinguishable", () => {
  ...
  it("success has branch set; error has errorPackage set; escalate has neither branch nor errorPackage", async () => {
    // success
    const successBackend = new ErrorEdgeBackend();
    ...
    // error (process crash)
    const errorBackend = new ErrorEdgeBackend();
    ...
    // escalate
    const escalateBackend = new ErrorEdgeBackend();
