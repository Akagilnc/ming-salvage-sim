...
### 新增
- **#683 额度探针状态机**：idle 超阈先探 pool 额度再判 hang；429 → `quota_wait_for_reset` ledger（含 resetAt，不杀 worker）；探针通过/网络错误 → fail-safe hang（只杀该实例 pid 树）。per-pool 配置（zai chat / opencode PONG / grok TBD）随 model ref 映射。自动重派属 #686。
