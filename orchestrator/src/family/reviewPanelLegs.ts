...
/** Durable evidence is reusable only by the exact current court generation. */
export function admissibleDurablePanelLegEvidence(
  ...
  if (durableHead.length === 0 || durableHead !== currentHead) return undefined;
  ...
  if (
    normalizeCourtGeneration(evidence.courtGeneration) !==
    normalizeCourtGeneration(scope.courtGeneration)
  ) {
    return undefined;
  }
...
/**
 * Family adapter over the unified review-panel mechanism. Existing evidence is
 * opaque court cargo; otherwise fresh family-scoped legs are dispatched.
 */
export async function ensureFamilyCmrPanelEvidence(input: {
  ...
  const round = await dispatchReviewPanelLegs({
    legs: input.legs,
    scope: { kind: "family", pass: input.cmrPass ?? "correctness" },
    dispatch: async (spec) => ({
      kind: "leg_result",
      result: await input.dispatch(spec),
    }),
  });
  if (round.kind !== "round") {
    throw new Error("family review-panel adapter received unexpected seat control");
  }
  return {
    panelLegTransports: round.transports,
    panelLegSkippedLegs: round.skippedLegs,
  };
}
