...
5. **盯文**：用测试断言文档正文——soul / ADR / CONTEXT.md / prompt 及一切
   md 的内容字句，正向钉、负向钉同罪。测试对文档最多只许查文件存在与否。
   被盯文档一字不动。判据：文档正文随便改、测试永不红（动态拼接 /
   机制守卫）＝合法；改正文会红＝盯文。交卷 token 以 `unconstitutional`
   代行（本禁令即 owner 拍定宪法），判词点名「盯文」。
