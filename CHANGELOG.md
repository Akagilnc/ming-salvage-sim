...
### Added
- **#1836 一次调用演完整场召对**：玩家可在同一场景连续召见、追问和宣人；模型按在场人物分别读取其可知材料，沿真实 Web/CLI 会话入口持续回应，不再为每句话拆出并行分类器。
- **#1837 / #1838 / #1839 召对转译落账**：回话后的结构化转译可登记交办、应允、分段与可闻性、人物进出、御前主角、边事件、公开说法和当场实况，并统一交给既有声明分派器逐项成案；所有结果绑定源轮，撤回时可按同一来源逆转。

### Changed
- **#1842 转译后台化**：玩家回话不再等待转译；封夜和过月只等待当前存档的后台任务，模型耗尽时保留可查询、可重试的待补状态，恢复后从原边界继续。
- 本场已说内容按源轮截止，场景材料按人物私有目录供给；召对暂存、案卷幂等键和第四类声明沿同一事务与来源边界落账。

### Fixed
- 修复跨存档同夜号/同轮号互相等待或撤回、未知声明键在分派前静默丢失、源轮查询异常被洗成空输入，以及封夜屏障未清仍继续推进的问题。
- 修复快速失败的后台 Future 在登记完成回调时重入非重入锁而偶发永久卡死；并发测试改用事件与结构化状态判定，失败路径会可靠释放并响亮收尾。

## [0.55.0.0] - 2026-09-12

### Added
- **#1832 / #1834 分层材料目录**：人物按现职、公开说法、亲历与密令排除读取各自材料；继任者可接续本衙门公档而不继承前任私密，世界推演者另有全盘面、事务、经历、公开层与历月邸报目录。
- **#1835 声明契约与分派器**：召对和过月可共用同一份结构化声明，逐项路由交办、拨帑、应允、当场实况、公开说法、进出场、分段、边事件与入册；错误项单独留痕，合法项继续落地，夜里产物可暂存、作废并按旨序幂等结算。

### Changed
- 材料发布、目录索引和读取统一为同一套隔离接口；密令人物/职位黑名单优先于公开层和衙门底账，御前近臣不再获得目标人物的账本真值。
- 上一份邸报改为完整传递，供后续叙事承接，不再只保留尾段。

### Removed
- **#1833 供料硬上限与旧路退役**：移除最近 20 条见闻、邸报尾段、每轮 5 次材料读取等限制，并删除已绕开的全知 builder 与旧按需读取工具，不新增替代上限。

## [0.54.0.0] - 2026-09-11

### Added
- **#1828 / #1829 世界记录分层**：文字事实成为只追加、带月份并可挂人物、军队、地区或事务的一等记录；公开说法独立进入角色见闻的公开层，不再冒充世界实况。
- **#1831 事务记录与指向**：案卷、人物经历、局势和文字事实可共同指向同一事务；事务归并与了结由 LLM 结构化声明，代码只维护引用与一致性。
- **#1830 角色材料目录**：人物调用改从中间层生成的索引化材料目录按需读取；CLI 与 API 共用同一材料边界，保留各 CLI 后端并以只读约束运行。

### Changed
- 大臣召对的开场上下文缩为身份、在场者、日期、经手事务摘要和本场对话；其余材料由角色按需读取，避免整份世界状态注入。
...
### Fixed
- **#1699 月末失败时仍能看清说明并继续操作**：结算错误提示不再遮住御笔区与盖玺按钮；常规与窄屏布局都会保留完整提示、输入区和操作区。
- **#1566 远地大臣也能收到玩家从「下密令」入口发出的密令**：密令按钮会把明确意图随消息传到后端，绕过只适用于殿上召对的地点闸门，并沿既有密令确认与入账流程处理；普通召对、拟旨、任命和承诺仍保持原有互斥规则。

## [0.52.0.2] - 2026-08-30

### Fixed
- **#1685 拟诏台手动新增的地域定向诏书草案不再因地域字段矛盾而阻断颁诏**：入卷前由 target_kind 写 single；正确组合可正常封存并推进月份，非法目标等仍明确失败且不生成残缺卷宗。
- **#1700 月末重试成功后不再弹出伪造的内部说明邸报**：邸报生成失败会保留原错误与重试边界，不再用固定 fallback 冒充成功；重试真正完成后，玩家看到的是生成成功的世界内叙事。

## [0.52.0.0] - 2026-08-30

### Fixed
- **#1682 月末亲裁拨帑按所选账户准确落账**：玩家选择内帑方案后会生成可核验的专属案卷，银两只从内帑即时拨出一次；选择国库方案时仍保留待核的国库拨款，不会误扣内帑。与本次亲裁无关的待决案卷也会原样保留。
- 拨帑方案的金额现在严格接受整数；字符串、布尔值与小数不会再被静默转换，客户端提交内容也不能覆盖服务端保存的权威方案。

## [0.51.0.3] - 2026-08-30

### Fixed
- **#1560 空拟诏台不再一键跳过整月**：没有草案或其他待结算事项时，主按钮保持禁用且不发结算请求；已有草案、待成案旨意、待处理召对事项或待办密令时，仍沿盖玺流程正常结算过月；仅有失败密令时经确认走 advance_without_edict。

## [0.51.0.1] - 2026-08-30

### For contributors
- **#1399 补齐召对失败的真实 App 回归契约**：测试现在贯穿结构化 SSE 错误、前端 hooks 与玩家警示，确保玩家只看到可理解提示，同时底层诊断仍保留错误码与服务方原文。

## [0.50.0.1] - 2026-08-30

### Fixed
- **#1399 召对失败不再向玩家泄漏内部错误码**：失败提示只显示可理解、可重试的系统说明；错误类型与服务方诊断仍完整保留在结构化错误详情中，既有原问重试入口保持不变。

## [0.49.15.0] - 2026-08-30

### Fixed
- **#1503 手工拟旨与一话两旨的拨饷载荷不再丢失**：手工下旨和召对中同句要求“拨饷并另行清查”时，拨饷金额、账户、用途、军队目标与颁布方式会沿同一字段登记完整进入案卷；太仓会在成案边界归一为国库，颁布后同回合恰扣一次国库并销减军队欠饷，另一道清查旨仍保持独立。残缺载荷、非法账户或非法颁布方式继续响亮拒绝且不推进回合。

## [0.49.14.2] - 2026-08-30

### Fixed
- **#1687 游戏内重开新局不再被旧档后台活动卡住**：新局完成后会先释放旧档写闸并切换到新档，再异步补做开局追赶；若切换失败则恢复旧档并明确结束本次请求。旧档仍在后台运行的活动不再永久阻塞新档与后续请求。

## [0.49.14.1] - 2026-08-30

### Fixed
- **#1376 专题查核密令过月不再丢失**：结构化抽取会把专题查核明确写成调查对象、正数事实目标和效果符号，并让交付单位保持为空；普通交付差务仍须提供规范单位与对应身份字段。玩家未明确准驳时，专题查核可沿既有“过月默认准”链入簿，不再因互相矛盾的必填条件触发失败计数。
...
### Fixed
- **#1624 批红结构化旨意契约（已由未发布条修正）**：旧文曾写「按最终目标类型统一写出地域范围」；现行为是显式组合校验、禁止静默改写施行范围，三入口共用同一契约真源。
