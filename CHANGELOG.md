...
### 变更
- 火耗对账 oracle 同步改为 `(正赋_o+三饷)×火耗率`，且 `官民田_o` 从开账+诏令独立重放（不再读清丈后的运行时税基）——清丈类「两侧同搬」篡账现在被债务+C 两层 oracle 当场咬死。
- 全部 value golden 钉死 `省库库银` 与 `官民田/隐田` 末态；raise-golden 升级为验证具体守门消息；spike FAIL 时退出码为 1（自动化不再假绿）。
