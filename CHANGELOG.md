...
### Added
- **#693 人物底座补齐阴谋与出身转变字段**：新开局及当前格式恢复会完整承载 `intrigue` / `defected_from`，勋戚种子与所有在朝大臣档案同步补齐；旧存档兼容及派系维护写权仍留给后续专票。
- **#702 信用事件成为忠诚度唯一机械写源**：既有四类信用事件按事件编号只消费一次，支持当前格式恢复和外层事务回滚；旧版“评定”动作退役，不再平行直写忠诚度。
- **#727 大臣六轴统一走输入侧定性投影**：人物上下文、模拟名册与玩家事件复用同一套五档词带；删除对模型自由散文的扫描、擦洗和盯文测试。
