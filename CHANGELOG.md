...
### 新增
- **对话式拟旨**（#137，对齐 ADR 0006）：玩家在召对中用自然语言「拟旨吧 / 你拟一道旨」即可触发大臣草拟圣旨，进入与「拟旨」按钮前缀相同的 pending 确认流（应允 / 拒绝 / 不回），不再必须点按钮；「补充」走同一条 pending 原地更新（last-write-wins）。「不回 = 颁诏默认同意」路径在真实 Web 全栈接通（后端结算 → 前端 `pending_directive_count` → 拟诏按钮可用）。

### 修复
- pre-landing 跨模型评审（红队 + codex）收敛的若干正确性 bug：
