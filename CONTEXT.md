...
**确认闸门**:
让**LLM 从自然语言推断出的**写动作变成 durable 游戏状态的关口。两种形态：拟旨候选走**显式准/驳**（皇帝逐条接受或驳回）；密令更新/催办等、后宫调教这类结构化推断动作走**颁诏批量同意**（颁诏即同意，颁诏前未撤回即通过）。**显式前缀按钮例外**：玩家点「密令如下/拟旨如下」= 已明示意图，按既定例外直接落库，不过此闸门。通过闸门后动作才落库。
_Avoid_: approval mode、模型自检、逐条点准（结构化动作不逐条点）、把显式前缀按钮也当成需推断确认的

**颁诏**:
皇帝当{月}对本{月}所有待确认动作的一次性成案确认点。颁诏时，未撤回的结构化动作候选一并落库（不拒绝即允许）；颁诏后进入月末结算。
