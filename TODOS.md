...
- **为何 defer 不当场修**：稳健修法要按消息身份（message_id / 对象身份）剪枝，但 `_fail_incomplete_chat_turn(minister, turn_id, user_text)` 的**文本签名已被 3 个测试**（`tests/test_chat_stream_cancel.py`）锁定，且 `chat_history` 条目 schema（`{"role","content"}`）被前端序列化/agent 上下文多处消费——改成带 id 属跨切片接口/schema 设计决策，超出 #370/#378 切片范围。
