...
def test_nonstream_court_break_preclaims_close_barrier(web_game, monkeypatch):
    """#1842：非流式退朝真实入口须由后台排程接缝预领收夜屏障。"""
    game = web_game
    minister = _active_minister(game)
    ...
    queue = game._runtime_write_queue()
    observed_preclaims: list[bool] = []
    original_claim_barrier = queue.claim_barrier

    def _observe_claim_barrier():
        ticket = original_claim_barrier()
        observed_preclaims.append(
            ticket is not None and queue.has_open_barrier()
        )
        return ticket

    monkeypatch.setattr(queue, "claim_barrier", _observe_claim_barrier)

    async def _court_break() -> dict:
        async with _client() as client:
    ...
    payload = asyncio.run(_court_break())

    assert payload.get("court_action") == "court_break", payload
    assert observed_preclaims == [True]
