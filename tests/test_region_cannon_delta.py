...
def test_city_cannon_capped_at_zero_for_low_city_level(game):
    ...
    rows = db.conn.execute(
        ...
        "WHERE region_id='dongjiang_area' AND field='cannon'"
    ).fetchall()
    ...
    assert "上限" in rows[0]["reason"], "痕迹缘由须点明被城防上限拦截"
...
def test_zero_cannon_request_leaves_no_log(game):
    ...
    rows = db.conn.execute(
        "SELECT 1 FROM region_logs WHERE region_id='dongjiang_area' AND field='cannon'"
    ).fetchall()
