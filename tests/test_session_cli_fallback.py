...
def test_cli_recommendation_reaches_session_pending_action_through_real_agent(game, monkeypatch):
    """#493 F7：CLI envelope 经真实 agno tool 执行后，由 session 暂存荐人。"""
    ...
    minister = next(
        ...
        and db.list_recommendation_candidates(state, c.name)
    )
    candidate = db.list_recommendation_candidates(state, minister.name)[0]
    model = cb.CliChat(id="test-recommendation", backend="codex")
    calls = iter((
        (
            "臣荐此人。[[recommend_person:"
            + json.dumps({
                "name": candidate["name"],
                "target_office": "巡盐御史",
                "reason": "可堪任事",
    ...
    monkeypatch.setattr(model, "_call_cli", lambda _prompt: next(calls))
    agent = Agent(
        name=minister.name,
        model=model,
        tools=build_minister_tools(minister, CourtContext(state=state, db=db)),
        markdown=False,
    )
    ...
    result = GameSession.chat(sess, minister.name, "可荐何人巡盐？")

    assert result.pending_action_id
    pending = db.list_pending_actions(state.turn)
    assert len(pending) == 1
    payload = json.loads(pending[0]["payload_json"])
    assert payload["recommendation"]["recommender"] == minister.name
